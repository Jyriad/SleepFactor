-- Home dashboard habit counts must match manual journal habits only.
-- Exclude all automatic health-metric habits (keep in sync with services/healthMetricsService.js).
...
BEGIN
  ...
  WHERE h.user_id = p_user_id
    ...
    AND (h.is_custom = true OR h.name NOT IN (
      'Daily Steps', 'Active Energy Burned', 'Max Heart Rate', 'Resting Heart Rate',
      'Exercise Duration', 'Exercise Intensity Index', 'Walking Distance',
      'Time in Sunlight', 'Last meal time', 'Night Body Temperature'
    ))
  ...
  WHERE e.user_id = p_user_id
    ...
    AND m.slug NOT IN ('tiredness', 'dream_vividness');
...
COMMENT ON FUNCTION public.get_home_dashboard_data(uuid, date) IS
  'Returns home dashboard payload; habit_counts exclude automatic health metrics and inferred habits (manual journal only).';

-- Keep habit logging RPC aligned with the same manual-habit filter.
