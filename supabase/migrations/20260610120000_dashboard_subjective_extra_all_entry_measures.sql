-- Include subjective_score_entries for all measures except tiredness/dream_vividness
-- (those scores live on sleep_data). Previously only non-builtin custom rows were included,
-- so "Easily fell asleep" (ease_sleep) never appeared on the home dashboard after refresh.
...
COMMENT ON FUNCTION public.get_home_dashboard_data(uuid, date) IS
  'Returns home dashboard payload; sleep rows respect users.preferred_sleep_source (NULL = all sources) plus manual. last_night_subjective extra includes ease_sleep and custom entry-table measures.';
