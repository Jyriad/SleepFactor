-- Home streak: count consecutive *paired* nights (sleep row + habit activity that Insights would pair),
-- not merely days with habit logs/consumption.
-- Pairing matches app Insights: habit_logs on day D pair with sleep wake date D+1; quick_consumption/drug
-- consumption on day S pairs with sleep wake date S.
...
COMMENT ON FUNCTION public.get_home_dashboard_data(uuid, date) IS
  'Returns home dashboard payload: sleep_record, habit_counts (logs + quick_consumption + drug consumption events), streak (consecutive paired nights: sleep row + matching habit log previous calendar day or consumption same day, same habit filters as Insights), user_prefs, last_night_subjective (sleep row scores + extra custom subjective scores), logged_dates. Caller must be authenticated and p_user_id = auth.uid().';
