-- Habit logging UI treats both quick_consumption and drug habits as consumption-backed.
-- Dashboard RPC only counted quick_consumption, so "x out of y" undercounted when users
-- logged supplements/meds (drug type) via habit_consumption_events.
...
COMMENT ON FUNCTION public.get_home_dashboard_data(uuid, date) IS
  'Returns home dashboard payload: sleep_record, habit_counts (logs + quick_consumption + drug consumption events), streak, user_prefs, last_night_subjective (from p_date sleep row), logged_dates. Caller must be authenticated and p_user_id = auth.uid().';
